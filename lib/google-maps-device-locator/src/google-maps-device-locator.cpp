...
static char requestBuf1[256];
static char *requestCur;
static char *requestCur1;
static int numAdded = 0;
static int numAdded1 = 0;
...
void GoogleMapsDeviceLocator::publishLocation() {
	...
	if (scanData[0]) {
		...
		if (Particle.connected()) {
			Particle.publish("deviceLocatorGPS", scanData, PRIVATE);
		}
	}
}
...
const char *GoogleMapsDeviceLocator::gpsScan() {

	float acc = 1.0;

	snprintf(requestBuf1, sizeof(requestBuf1), "%s,%s,%s", String(_lat).c_str(), String(_lon).c_str(), String(acc).c_str());

	return requestBuf1;
}


void GoogleMapsDeviceLocator::update_lat(float lat) {
	_lat = lat * 0.0000001;
}

void GoogleMapsDeviceLocator::update_lon(float lon) {
	_lon = lon * 0.0000001;
}
